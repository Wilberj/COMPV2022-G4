...
class TableComponent extends HTMLElement {
	constructor(config = { Dataset: [], Funtions: [], ModelObject: {} }) {
		super();
		this.config = config;
						...
						const Dataset = this.config.Dataset.filter((element) => {
							for (const prop in element) {
								if (element[prop] != null) {
									if (
										element[prop].toString().toUpperCase().includes(ev.target.value.toUpperCase())
									) {
										return element;
									}
								}
		...
		Dataset.forEach((data, index) => {
			...
			if (this.config.Funtions) {
				...
				tr.append(
					Render.Create({
						tagName: "td",
						children: this.config.Funtions.map((f) => {
							return {
								tagName: "input",
								type: "button",
								className: "btn",
